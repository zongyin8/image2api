...
- `rejected`：余额不足、并发已满、敏感词、未知模型等在调用供应商前被拦截的请求。保留审计日志，但不计入生成失败、累计生成、API 调用或图片/视频计数。
- `failed`：已经进入生成执行后失败的请求，用于衡量供应商、账号池和系统的真实失败率。
- 管理后台成功率固定使用 `success / (success + failed)`，不把 `pending` 或 `rejected` 放进分母。
- 同一用户、来源和拦截原因在 5 分钟内只写一条 `rejected` 日志；HTTP 请求仍逐次返回原始错误，不影响客户端判断。
- 上游合并时不得把 `logRejectedEvent` 改回 `failed`，也不得让 `rejected` 增加 `stat_counters`。

### P0：全面替换前必须完成

1. 为 `n>1` 定义批量计费、逐张退款、部分成功返回和总超时，禁止继续静默只返回一张。
2. 统一 OpenAI 错误体，同时保留 `detail`，让 SDK 能稳定读取 `error.message/type/code`。
3. 客户端断线策略已拆分：前端/管理任务继续执行并可轮询；API Key 无存储请求随连接取消，使用独立记账上下文落失败状态和退款。
