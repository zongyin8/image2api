...
- **账号独立代理**:可给指定账号配置 HTTP / HTTPS / SOCKS5 出口,也可多选账号批量设置或清空;未配置时沿用节点全局代理
...
- 已有模型(如 `gpt-image-2`)无需重复新建同名自定义模型,上游留空支持模型即可按 id 自动接管;只有上游提供系统中不存在的新模型时,才需在模型管理里新增模型并配置类型 / 比例 / 分辨率·价 / 时长·价 / 参考图
- `gpt-image-2` 的调度是分辨率与容量感知的:1K 优先使用本地 ChatGPT 池,本地没有可用账号或所有账号的单任务并发槽已满时自动切到 custom;2K/4K 直接走 custom。这里检查的是 Redis 中的实时账号并发,不是只看数据库里的 `active` 状态
- 默认直连,也可按上游账号单独配置代理;上游可配权重与并发,与内置池统一调度
...
- 模型管理(普通价 + 代理价 + 别名) · 账号管理(批量导入 / 去重 / 额度 / 单号代理 / 批量编辑) · **并发分组** · **订单管理**(筛选 / 搜索 / 分页) · 全站日志 · 用户管理(设为代理 / 分配并发组 / 看累计充值 / 违禁触发次数) · CDK · 图片管理(多选批量删除 / zip 打包下载) · 展示位 · **站点公告** · 站点配置(含易支付、去AI特征开关与附加价格)
...
| 供应商 | 模型(示例) | 类型 |
|---|---|---|
| **Adobe Firefly** | firefly-image-5 · firefly-gpt-image-2 · flux-kontext-max · firefly-video · firefly-ray · gemini-veo31 · seedance-2.0 · seedance-2.0-fast | 图像 / 视频 |
...
| **自定义上游** | OpenAI 兼容 v1 图片端点 · Sora 兼容视频端点 | 图像 / 视频 |

> 模型由管理后台动态启用并定价,可随时增删。自定义上游支持 OpenAI/Sora 协议,按 model id 路由调用。
...
│   │   ├── krea/              Krea
│   │   ├── imagine/           Imagine.art
│   │   ├── custom/            自定义上游(OpenAI 兼容 v1,按 id 路由,支持账号独立代理)
